...
from robosuite.models.objects import (
    CroissantObject,
    PlateObject,      # your custom object
)
...
class ServeBread(ManipulationEnv):
    """
    ...
    def reward(self, action=None):
        ...
        # use a shaping reward
            ...
            dist = self._gripper_to_target(
                gripper=self.robots[0].gripper, target=self.bread.root_body, target_type="body", return_distance=True
            )
    ...
    def _load_model(self):
        ...
        #     material=redwood,
        # )
        self.bread = CroissantObject(name="croissant")
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.bread, self.plate],
        )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [bread_pos, bread_quat, plate_pos, plate_quat]
    ...
    def _reset_internal(self):
        ...
        # Fixed plate position
        plate_pos = np.array([
             0.0,                # x
            -0.2,               # y
            self.table_offset[2] + 0.06,
        ])

        # plate_quat = np.array([0.70710678, 0.70710678, 0.0, 0.0])
        plate_quat = np.array([1, 0, 0, 0])  # no rotation

        self.sim.data.set_joint_qpos(
            self.plate.joints[0],
            np.concatenate([plate_pos, plate_quat]),
        )
    ...
    def visualize(self, vis_settings):
        ...
        # Color the gripper visualization site according to its distance to the bread
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.bread)

    def _check_success(self):
        """
        Check if bread has been placed on the plate.

        Returns:
            bool: True if bread has been placed on the plate
        """
        bread = self.sim.data.body_xpos[self.bread_body_id]
        plate = self.sim.data.body_xpos[self.plate_body_id]
